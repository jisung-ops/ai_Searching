...
* **실시간 AI 스트리밍**: Google Gemini 1.5 Flash 모델과 Vercel AI SDK를 사용하여 지연 시간을 줄이고 부드러운 글자 쓰기(`smoothStream`) 효과를 적용했습니다. '내가 내일 날씨 어떨거같아'와 같은 구어체 질문 및 미지정 지역에 대한 정밀한 위치명 파싱, 그리고 최신 Vercel AI SDK 호환 UI Message Stream Fallback 프로토콜이 지원되어 안정적인 답변 출력을 보장합니다. 사용자의 글자 수나 조건 제한 요청에 정밀하게 대응하기 위한 자체 검증 프롬프트가 탑재되었습니다.
