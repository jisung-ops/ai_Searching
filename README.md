...
* **Direct URL 링크 입력 & 웹페이지 실시간 분석 결합 (Direct URL & Web Scraping Search)**: 검색어와 함께 특정 웹페이지 URL(뉴스, 블로그, 공식 문서 등)을 입력하거나 `[웹 링크 결합]` 버튼을 통해 추가하면, 입력창에 반응형 Glassmorphism URL 칩이 표시되고 서버 측 실시간 파서(`fetchUrlContent`)가 웹페이지 본문과 타이틀을 추출합니다. 지정된 웹페이지 정보와 최신 실시간 웹 검색(Tavily) 결과가 하이브리드로 결합되어 교차 분석 보고서 및 인라인 인용 툴팁이 생성됩니다.
